...
class AccessibilityCheckerHighlight {
	...
	descriptionOpen( dataId ) {
		...
		if ( matchingObj ) {
			...
			const editorLinkClass = window.elementorFrontend?.isEditMode?.() === true ? ' elementor-clickable' : '';
			...
			if ( isPro && hasExplanation ) {
				...
				content += `<div><a class="edac-highlight-panel-description-reference${ editorLinkClass }" href="${ matchingObj.link }" target="_blank" rel="noopener noreferrer">${ __( 'More Detailed Documentation', 'accessibility-checker' ) } ${ newWindowHtml }</a></div>`;
				content += `</div>`;
			} else {
				// Free: show a plain "How to Fix" link
				content += `<a class="edac-highlight-panel-description-reference${ editorLinkClass }" href="${ matchingObj.link }" target="_blank" rel="noopener noreferrer">${ __( 'How to Fix', 'accessibility-checker' ) } ${ newWindowHtml }</a>`;
			}
