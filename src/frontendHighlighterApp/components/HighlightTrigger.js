...
class HighlightTrigger extends HTMLElement {
		...
		template.innerHTML = `
			<style>
				...
				button {
					...
					display: block;
					cursor: pointer;
					background: transparent url("${ LOGO_DATA_URI }") center center no-repeat;
			...
			<button aria-haspopup="dialog" aria-label="${ ariaLabel }"></button>
		`;
	...
	_syncVisibility() {
		...
		this.style.display = isOpen ? 'none' : 'block';
	}

	_onClick = () => {
		this.dispatchEvent( new CustomEvent( 'edac-toggle-panel', {
			bubbles: true,
			composed: true,
		} ) );
	};
...
if ( ! customElements.get( 'edac-highlight-trigger' ) ) {
	customElements.define( 'edac-highlight-trigger', HighlightTrigger );
}

export { HighlightTrigger };
