/**
 * <edac-highlight-tooltip-button>
 *
 * One instance is created per flagged element on the page. Positioning is
 * owned by the caller (floating-ui reads getBoundingClientRect() on the
 * target element and on this custom element, both of which work the same
 * whether or not the element has a shadow root).
 *
 * Selection state is intentionally read via a host class rather than a
 * property so existing callers can keep doing
 * `tooltip.classList.add( 'edac-highlight-btn-selected' )` /
 * `document.querySelectorAll( '.edac-highlight-btn-selected' )` unchanged.
 */

const ICONS = {
	error: "data:image/svg+xml,%3Csvg width='46' height='46' viewBox='0 0 46 46' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Ccircle cx='23' cy='23' r='23' fill='white'/%3E%3Ccircle cx='23' cy='23' r='20' fill='%23B30F0F'/%3E%3Ccircle cx='23' cy='23' r='18' stroke='black' stroke-opacity='0.4' stroke-width='4'/%3E%3Ccircle cx='23' cy='23' r='13.435' transform='rotate(45 23 23)' fill='white'/%3E%3Crect x='27.0515' y='16.7132' width='3.16118' height='14.818' rx='1.58059' transform='rotate(45 27.0515 16.7132)' fill='%23B30F0F'/%3E%3Crect x='29.3566' y='27.1213' width='3.16118' height='14.818' rx='1.58059' transform='rotate(135 29.3566 27.1213)' fill='%23B30F0F'/%3E%3C/svg%3E",
	warning: "data:image/svg+xml,%3Csvg width='46' height='46' viewBox='0 0 46 46' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Ccircle cx='23' cy='23' r='23' fill='white'/%3E%3Ccircle cx='23' cy='23' r='20' fill='%23F3CD1E'/%3E%3Ccircle cx='23' cy='23' r='18' stroke='black' stroke-opacity='0.4' stroke-width='4'/%3E%3Cpath d='M21.4093 10.7551C22.1163 9.53061 23.8837 9.53061 24.5907 10.7551L34.3997 27.7449C35.1067 28.9694 34.223 30.5 32.8091 30.5H13.1909C11.777 30.5 10.8933 28.9694 11.6003 27.7449L21.4093 10.7551Z' fill='%23072446'/%3E%3Crect x='21.7755' y='14.8878' width='2.44898' height='9.18367' rx='1.22449' fill='%23F3CD1E'/%3E%3Crect x='21.7755' y='25.1429' width='2.44898' height='2.44898' rx='1.22449' fill='%23F3CD1E'/%3E%3C/svg%3E",
	ignored: "data:image/svg+xml,%3Csvg width='46' height='46' viewBox='0 0 46 46' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Ccircle cx='23' cy='23' r='23' fill='white'/%3E%3Ccircle cx='23' cy='23' r='20' fill='%23072446'/%3E%3Ccircle cx='23' cy='23' r='18' stroke='black' stroke-opacity='0.4' stroke-width='4'/%3E%3Cpath opacity='0.991' fill-rule='evenodd' clip-rule='evenodd' d='M21.3204 10.0131C24.4685 9.85403 26.8164 11.1441 28.3641 13.8833C29.3235 15.7433 29.7363 17.7213 29.6026 19.8175C29.7265 22.1744 30.1738 24.4621 30.9443 26.6806C28.5079 26.3678 26.7534 27.3139 25.6808 29.5187C25.2423 30.7036 25.2337 31.8904 25.655 33.0793C22.7481 33.0879 19.8411 33.0793 16.9343 33.0535C15.5357 33.0185 14.1597 32.8293 12.8061 32.4859C12.4965 32.3655 12.1869 32.245 11.8772 32.1246C11.4521 31.8936 11.1597 31.5496 11 31.0926C11.028 30.9008 11.0882 30.7202 11.1806 30.5508C11.4042 30.2583 11.6622 30.0003 11.9546 29.7767C12.5331 29.0763 12.9976 28.3023 13.3479 27.4546C14.018 25.7536 14.4997 23.9991 14.7928 22.1912C14.9679 20.4219 15.1055 18.6502 15.2056 16.8762C15.642 14.3279 16.9235 12.3412 19.0499 10.9161C19.7329 10.4878 20.4725 10.2126 21.2688 10.0905C21.2975 10.0708 21.3147 10.045 21.3204 10.0131Z' fill='white'/%3E%3Cpath opacity='0.964' fill-rule='evenodd' clip-rule='evenodd' d='M29.6284 27.3514C31.7421 27.2251 33.1955 28.1367 33.9888 30.0864C34.5213 32.1412 33.9107 33.7495 32.1569 34.9112C30.277 35.7907 28.6 35.5069 27.1257 34.0597C26.0698 32.7639 25.8462 31.3362 26.4549 29.7767C27.111 28.4386 28.1688 27.6301 29.6284 27.3514ZM28.8544 29.6735C29.0243 29.6611 29.1792 29.7041 29.3188 29.8025C29.6026 30.0864 29.8864 30.3702 30.1702 30.654C30.4712 30.353 30.7723 30.0519 31.0733 29.7509C31.7128 29.6166 31.945 29.866 31.7699 30.4992C31.4767 30.7838 31.1929 31.0761 30.9185 31.3764C31.1929 31.6767 31.4767 31.9691 31.7699 32.2536C31.9585 32.7021 31.8209 32.9773 31.3571 33.0793C31.2556 33.0771 31.1609 33.0513 31.0733 33.0019C30.7533 32.7334 30.4523 32.4496 30.1702 32.1504C29.8354 32.4508 29.5 32.7518 29.164 33.0535C28.7639 33.1266 28.5231 32.9632 28.4416 32.5633C28.4621 32.4757 28.4879 32.3897 28.519 32.3052C28.8293 32.0035 29.1303 31.6939 29.422 31.3764C29.1332 31.0617 28.8322 30.7521 28.519 30.4476C28.3862 30.0797 28.498 29.8217 28.8544 29.6735Z' fill='white'/%3E%3Cpath opacity='0.931' fill-rule='evenodd' clip-rule='evenodd' d='M19.6692 33.9565C21.5616 33.9393 23.4536 33.9565 25.3454 34.0081C24.6552 35.5603 23.4684 36.2053 21.7849 35.9432C20.719 35.6598 20.0137 34.9975 19.6692 33.9565Z' fill='white'/%3E%3C/svg%3E",
};

class HighlightTooltipButton extends HTMLElement {
	static get observedAttributes() {
		return [ 'rule-type', 'aria-label', 'aria-expanded' ];
	}

	constructor() {
		super();
		this.attachShadow( { mode: 'open', delegatesFocus: true } );
	}

	connectedCallback() {
		if ( ! this.shadowRoot.firstChild ) {
			this.render();
		}
		this.button.addEventListener( 'click', this._onClick );
	}

	disconnectedCallback() {
		this.button.removeEventListener( 'click', this._onClick );
	}

	attributeChangedCallback( name, oldValue, newValue ) {
		if ( ! this.button || oldValue === newValue ) {
			return;
		}
		if ( name === 'rule-type' ) {
			this.button.className = newValue || 'error';
		} else if ( name === 'aria-label' ) {
			this.button.setAttribute( 'aria-label', newValue || '' );
		} else if ( name === 'aria-expanded' ) {
			this.button.setAttribute( 'aria-expanded', newValue || 'false' );
		}
	}

	render() {
		const ruleType = this.getAttribute( 'rule-type' ) || 'error';
		const ariaLabel = this.getAttribute( 'aria-label' ) || '';
		const ariaExpanded = this.getAttribute( 'aria-expanded' ) || 'false';

		const template = document.createElement( 'template' );
		template.innerHTML = `
			<style>
				:host {
					all: initial;
					display: block;
					position: absolute;
					z-index: 2147483646;
				}

				button {
					all: unset;
					width: 40px;
					height: 40px;
					display: block;
					font-size: 0;
					border-radius: 50%;
					margin: 5px;
					cursor: pointer;
					background-size: 40px 40px;
					background-position: center center;
					background-repeat: no-repeat;
				}

				button.error { background-image: url("${ ICONS.error }"); }
				button.warning { background-image: url("${ ICONS.warning }"); }
				button.ignored { background-image: url("${ ICONS.ignored }"); }

				:host( .edac-highlight-btn-selected ) button,
				button:hover,
				button:focus {
					outline: solid 5px rgba(0, 208, 255, .75);
				}
			</style>
			<button class="${ ruleType }" aria-label="${ ariaLabel }" aria-expanded="${ ariaExpanded }" aria-haspopup="dialog"></button>
		`;

		this.shadowRoot.appendChild( template.content.cloneNode( true ) );
		this.button = this.shadowRoot.querySelector( 'button' );
	}

	_onClick = ( e ) => {
		e.stopPropagation();
		this.dispatchEvent( new CustomEvent( 'edac-open-issue', {
			bubbles: true,
			composed: true,
			detail: { issueId: this.issueId },
		} ) );
	};

	get issueId() {
		return this.getAttribute( 'issue-id' );
	}

	set issueId( value ) {
		this.setAttribute( 'issue-id', value );
	}

	focus( options ) {
		this.button?.focus( options );
	}
}

if ( ! customElements.get( 'edac-highlight-tooltip-button' ) ) {
	customElements.define( 'edac-highlight-tooltip-button', HighlightTooltipButton );
}

export { HighlightTooltipButton };
