...
class Scans_Stats {
	...
	public function summary() {
		...
		$scannable_posts_count = Scannable_Posts::get_scannable_posts_count();
		...
		$data['scannable_post_types_count'] = (int) count( Scannable_Posts::get_scannable_post_types() );
		...
		$warning_issues_query      = new Issues_Query(
			array(
				'post_types' => Scannable_Posts::get_scannable_post_types(),
		...
		if ( $data['posts_scanned'] > 0
			&& ! empty( Scannable_Posts::get_scannable_post_types() )
			&& ! empty( Scannable_Posts::get_scannable_post_statuses() )
		) {
			...
			Helpers::array_to_sql_safe_list(
				Scannable_Posts::get_scannable_post_types()
			) . ') and post_status IN(' . Helpers::array_to_sql_safe_list(
				Scannable_Posts::get_scannable_post_statuses()
			) . ')';
