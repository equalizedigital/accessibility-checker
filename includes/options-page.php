 ...
 */
function edac_disabled_rules_cb() {
	...
	if ( isset( $wp_filter['edac_filter_register_rules'] ) ) {
		foreach ( $wp_filter['edac_filter_register_rules']->callbacks as $callbacks ) {
			foreach ( $callbacks as $callback ) {
				...
				if ( ! ( is_array( $fn ) && $fn[0] instanceof \EqualizeDigital\AccessibilityChecker\Admin\AdminPage\RulesPage && 'apply_disabled_rules_setting' === $fn[1] ) ) {
 ...
 */
function edac_sanitize_disabled_rules() {
